...
public class PacketReader
{
    ...
    public void startup() throws XMPPException
    {
        ...
        LogUtils.i(TAG, "calling startup ");
    ...
    void startCheckConnectionProcess()
    {
        ...
        {
            checkConnectionTask task = new checkConnectionTask(keepAliveInterval);
            connectionCheckThread = new Thread(task);
            task.setThread(connectionCheckThread);
            connectionCheckThread.setDaemon(true);
            connectionCheckThread.setName("Connection Checking Thread");
    ...
    private class checkConnectionTask implements Runnable
    {
        ...
        public checkConnectionTask(int delay)
        {
            this.delay = delay;
        }
        ...
        public void run()
        {
            ...
            while (!done && connectionCheckThread == thread)
            {
                if (delay * 2 > lastRecTime)
                {
                    notifyConnectionError(new Exception("no message from server"));
                }
                try
                {
                    Thread.sleep(delay);
