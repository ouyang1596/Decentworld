...
public class PopupWindows extends PopupWindow {
	...
	private boolean mIfSingle;// 判断图片是多选还是单选
	private Fragment mFragment;

	public PopupWindows(Context context, List<ImageItem> datalist, View parent, boolean ifSingle) {
		initView(context, datalist, parent, ifSingle);
	}

	public PopupWindows(Context context, Fragment fragment, List<ImageItem> datalist, View parent, boolean ifSingle) {
		mFragment = fragment;
		initView(context, datalist, parent, ifSingle);
	}

	private void initView(Context context, List<ImageItem> datalist, View parent, boolean ifSingle) {
		mIfSingle = ifSingle;
		mContext = context;
		mDataList = datalist;
		...
		bt2.setOnClickListener(new OnClickListener() {
			public void onClick(View v) {
				...
				if (mIfSingle) {
					intent = new Intent(mContext, ImageChooseSingleActivity.class);
					if (null != mFragment) {
						mFragment.startActivityForResult(intent, ALBUM);
					} else {
						activity.startActivityForResult(intent, ALBUM);
					}
				} else {
					intent = new Intent(mContext, ImageChooseActivity.class);
	...
	public void takePhoto() {
		...
		if (null != mFragment) {
			mFragment.startActivityForResult(openCameraIntent, TAKE_PICTURE);
		} else {
			Activity activity = (Activity) mContext;
