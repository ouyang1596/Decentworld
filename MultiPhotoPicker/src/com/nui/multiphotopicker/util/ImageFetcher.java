...
public class ImageFetcher {
	...
	public List<ImageBucket> getImagesBucketList(boolean refresh) {
		...
		for (int i = 0; i < tmpList.size(); i++) {
			if ("全部".equals(tmpList.get(i).bucketName)) {
				ImageBucket bucket = tmpList.get(i);
				tmpList.remove(i);
	...
	private void buildImagesBucketList() {
		...
		try {
			...
			if (cur.moveToFirst()) {
				...
				ImageBucket imageBucket = mBucketList.get("1000");
				if (null == imageBucket) {
					imageBucket = new ImageBucket();
					imageBucket.bucketName = "全部";
					imageBucket.imageList = new ArrayList<ImageItem>();
					mBucketList.put("1000", imageBucket);
				}
				do {
	...
	private void getThumbnailColumnData(Cursor cur) {
		if (cur.moveToFirst()) {
			...
			do {
				...
				Log.i("bm", "image_path--" + image_path);
			} while (cur.moveToNext());
		}
	}
}
