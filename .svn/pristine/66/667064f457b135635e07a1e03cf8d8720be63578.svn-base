...
public class RechargePayMethodActivity extends BaseFragmentActivity {
	...
	public void init() {
		...
		llWX.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View view) {
				...
				// else if (isGuarantee) {
				// getGuaranteeCharge(WX, moneyCount, dwID, phoneNum, wxHandle);
				// }
				else {
					getCharge(WX, moneyCount, DecentWorldApp.getInstance().getDwID(), wxHandle);
				}
	...
	private void RGetIntent() {
		...
		// }
			...
			moneyCount = getIntent().getStringExtra(RechargeAliPayWXActivity.MONEY);
		}
	}
	 ...
	 * @param handler
	 */
	public void getGuaranteeCharge(int payType, String amount, String dwID, String phoneNum, Handler handler) {
		HashMap<String, String> map = new HashMap<String, String>();
		map.put("type", String.valueOf(payType));
		map.put("amount", amount);
		map.put("dwID", dwID);
