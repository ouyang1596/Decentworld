    ...
    public View getView(final int position, View view, ViewGroup arg2)
    {
        ...
        if (pickType == PickContactActivity.PICK_TYPE_SINGLE)
        {
            ...
            viewHolder.ll_pick_contact.setOnClickListener(new OnClickListener()
            {
                ...
                {
                    LogUtils.i(TAG, "选择的联系人DwID = " + list.get(position).getFriendID());
                    ArrayList<String> result = new ArrayList<String>();
                    result.add(list.get(position).getFriendID());
                    Intent intent = new Intent();
                    intent.putStringArrayListExtra(PickContactActivity.CONTACT_DWID, result);
