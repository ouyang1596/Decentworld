...
import cn.sx.decentworld.dialog.incom.SetIncomePwdDialog;
import cn.sx.decentworld.dialog.incom.SetIncomePwdDialog.SetIncomePwdListener;
...
public class RechargeBenefitActivity extends BaseFragmentActivity implements OnClickListener {
    ...
    {
        SetIncomePwdDialog dialog = new SetIncomePwdDialog();
        dialog.setListener(new SetIncomePwdListener()
        {
            @Override
            public void onSubmit(String password)
            {
                ...
                new BenefitEngine().setBenefitPwd(dwID, password, new NetCallback()
                {
                    @Override
                    public void onSuccess(String msg)
                    {
                        setPwdHandler.sendEmptyMessage(1);
                    }
                    
                    @Override
                    public void onFailure(String cause)
                    {
                        setPwdHandler.sendEmptyMessage(0);
                    }
                });
            }
        });
    ...
    Handler setPwdHandler = new Handler()
    {
        public void handleMessage(android.os.Message msg) {
            switch (msg.what)
            {
