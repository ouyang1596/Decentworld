    ...
    */ 
    ...
    private Packet nextPacket() {
        ...
        while (!done && (packet = queue.poll()) == null) {
            try {
                ...
                // Do nothing
            	LogUtils.i(TAG, "InterruptedException异常："+ie.toString());
	...
	private void writePackets(Thread thisThread)
	{
		try
		{
			...
			{
				e.printStackTrace();
				LogUtils.i(TAG, "writePackets...Exception异常：" + e.toString());
			}
		...
		{
			// 当断开网络时
			LogUtils.i(TAG, "writePackets...IOException异常：" + ioe.toString());
			if (!done)
			{
				done = true;
