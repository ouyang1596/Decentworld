...
public class ChatRoomMeActivity extends BaseFragmentActivity implements
		OnClickListener {
	...
	private Handler mHandler = new Handler() {
		public void handleMessage(android.os.Message msg) {
			switch (msg.what) {
			case GET_CHATROOMS:
				List<MyChatRoom> chatrooms = (List<MyChatRoom>) JsonUtils
						.json2BeanArray(msg.obj.toString(), MyChatRoom.class);
				mChatRooms.clear();
	...
	private void setHead() {
		...
		if (ifHasSubject) {
			ImageLoaderHelper.mImageLoader.displayImage(headIcon, ivHead,
					ImageLoaderHelper.mOptions);
		} else {
			ImageLoaderHelper.mImageLoader.displayImage(ImageUtils
					.getIconByDwID(DecentWorldApp.getInstance().getDwID(),
	...
	public void onClick(View view) {
		...
		case R.id.tv_edit_self_info:
			...
			if (null == mChatRooms || mChatRooms.size() <= 0) {
				...
				intent.putExtra(CHATROOM_NICKNAME,
						mChatRooms.get(0).ownerNickName);
			}
			startActivity(intent);
			break;
		case R.id.iv_back:
	...
	class ChatRoomsAdapter extends BaseAdapter implements OnClickListener {
		...
		public View getView(int position, View con, ViewGroup arg2) {
			...
			if (null == con) {
				...
				vh.ivOwnerIcon = (CircularImageView) con
				...
				vh.tvSubjectList = (ImageView) con
						.findViewById(R.id.iv_mysubject_list);
				vh.tvChargeAmount = (TextView) con
						.findViewById(R.id.rb_fee_scale);
				vh.ivRoomBackground = (ImageView) con.findViewById(R.id.iv_bg);
				vh.ivNewTheme = (CircularImageView) con
						.findViewById(R.id.iv_new_theme);
			...
			if (CommUtil.isNotBlank(chatroom.ownerIcon)) {
				ImageLoaderHelper.mImageLoader.displayImage(chatroom.ownerIcon,
						vh.ivOwnerIcon, ImageLoaderHelper.mOptions);
			} else {
			...
			if (CommUtil.isNotBlank(chatroom.roomBackground)) {
				ImageLoaderHelper.mImageLoader.displayImage(
						chatroom.roomBackground, vh.ivRoomBackground,
						ImageLoaderHelper.mOptions);
			} else {
			...
			if (null == chatroom.subjectAmount
					|| Float.valueOf(chatroom.subjectAmount) <= 1) {
		...
		class ViewHolder {
			TextView tvSubjectName, tvSelfIntroduce, tvOnLineNum,
					tvSelfNickName, tvChatRoomEnter, tvChargeAmount;
		...
		public void onClick(View view) {
			...
			case R.id.iv_mysubject_list:
				intent = new Intent(mContext,
						ChatRoomMySubjectListActivity_.class);
				intent.putExtra(ROOMID, item.roomID);
				startActivity(intent);
				break;
			}
