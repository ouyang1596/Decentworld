...
public class ConversationListAdapter2 extends BaseAdapter {
	...
	public ConversationListAdapter2(Context context,
			List<ConversationList> messageLists) {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		if (convertView == null) {
			...
			convertView = inflater.from(context).inflate(
					R.layout.item_stranger_recorde, null);
			...
			holder.tv_content = (TextView) convertView
					.findViewById(R.id.tv_fragment_chat_chat_content);
			holder.tv_time = (TextView) convertView
					.findViewById(R.id.tv_fragment_chat_chat_time);
			holder.unread_msg_number = (TextView) convertView
					.findViewById(R.id.tv_fragment_chat_chat_unread_msg_number);
			// holder.chat_msg_state = (ImageView) convertView
			// .findViewById(R.id.iv_fragment_chat_chat_msg_state);
			// holder.chat_unremind = (ImageView) convertView
			// .findViewById(R.id.iv_fragment_chat_chat_unremind);
		...
		if (CommUtil.isNotBlank(messageList.getIcon())) {
			ImageLoaderHelper.mImageLoader.displayImage(messageList.getIcon(),
		...
		// holder.tv_time.setText(messageList.getTime());
		holder.tv_time.setText(TimeUtils.getDate_refer_Time_stamp(messageList
				.getTime()));
		...
		// if (messageList.getMsg_state() != 1) {
		// holder.chat_msg_state.setVisibility(View.VISIBLE);
		// } else {
		// holder.chat_msg_state.setVisibility(View.INVISIBLE);
		// }
		// if (messageList.getChat_unremind() != 1) {
		// holder.chat_unremind.setVisibility(View.VISIBLE);
		// } else {
		// holder.chat_unremind.setVisibility(View.INVISIBLE);
		// }
		if (messageList.getSetToTop() != 1) {
			holder.list_item_layout
					.setBackgroundResource(R.drawable.chat_lv_top_item_selector);
		} else {
			holder.list_item_layout
					.setBackgroundResource(R.drawable.chat_lv_item_selector);
		}
		return convertView;
	}
