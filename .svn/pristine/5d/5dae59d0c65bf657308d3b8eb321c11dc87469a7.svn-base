...
public class XMPPConnection extends Connection
{
     ...
     * server.
     */
    private boolean authenticated = false;// 标记当前用户是否被服务器验证过
    /**
     ...
     * by the server. If the server supports SASL authentication then the user
     * will be authenticated using SASL if not Non-SASL authentication will be
     * tried. If more than five seconds (default timeout) elapses in each step
     * of the authentication process without a response from the server, or if
     * an error occurs, a XMPPException will be thrown.
     * <p>
     * 
     * Before logging in (i.e. authenticate) to the server the connection must
     * be connected. For compatibility and easiness of use the connection will
     ...
     *             serrver.
     */
    public synchronized void login(String username, String dwId, String password, String resource) throws XMPPException
    {
        if (!isConnected())
        {
        ...
        {
            // ---------
            // this.user = dwId + "@" + getServiceName();
            // Log.i(Constants.TAG,"user: "+ this.user);
        ...
        /** 当配置Configuration时指定setSendPresence为true时，那么登录完成会自动发送
         * Presence到服务器，这样就可以自动接收服务器推送的离线消息，如果设置为false，那么就登录后
         * 就不会发送Presence到服务器，这时可以手动发送**/
        ...
        /** 授权后存储必要的信息，用于将来重新使用 **/
        config.setLoginInfo(username, dwId, password, resource);
        ...
        // logined and notify packetcollector threads
     ...
     * will likely be in the form "server/123ABC" (where "123ABC" is a random
    ...
    {
        CrashHandler.getInstance().setExtraInfo(TAG + " sendPacket", packet.toXML());
        LogUtils.i(TAG, packet.toXML());
        if (!isConnected())
        {
            throw new IllegalStateException("Not connected to server.");
        }
        ...
        // LogUtils.i(TAG, "将要发送的Packet为："+packet.toXML());
    ...
    private void initConnection() throws XMPPException
    {
        ...
        try
        {
            ...
            packetReader.startCheckConnectionProcess();
            if (isFirstInitialization)
            {
    ...
    void proceedTLSReceived() throws Exception
    {
        ...
        else
        {
            ...
            else if (config.getKeystoreType().equals("PKCS11"))
            {
                try
                {
                    ...
                    this.config.getCallbackHandler().handle(new Callback[]
                    { pcb });
        ...
        { new ServerTrustManager(getServiceName() , config) }, new java.security.SecureRandom());
        Socket plain = socket;
    ...
    public void connect() throws XMPPException
    {
        ...
        if (connected && wasAuthenticated)
        {
            ...
            try
            {
                ...
                {
                    LogUtils.i("Xmppconnection....", "dwID=" + config.getDwID());
                    login(config.getUsername(), config.getDwID(), config.getPassword(), config.getResource());
                }
                packetReader.notifyReconnection();
            }
