...
public class ChatSingleEngine
{
    ...
    private ChatManager creatChatManager()
    {
        ...
        LogUtils.d(TAG, "creatChatManager " + connection.toString() + "已经连接和授权");
        LogRecord.getInstance().saveExceptionLog(TAG, "creatChatManager...isConnected=" + connection.isConnected() + ",isAuth=" + connection.isAuthenticated());
     ...
     */
    public void sendTextMessage(DWMessage dwMessage, long maxID) throws NotConnectedException
    {
        // 构造Message
        ...
        try
        {
            ...
            //
            LogUtils.i(TAG, "sendTextMessage...connection=" + connection.toString() + "\n" + ",dwMessage=" + dwMessage.toString());
            // 将信息保存到Bmob
            LogUtils.w(TAG, "sendTextMessage...发送消息之前:" + "\ndwMessage.toString()=" + dwMessage.toString() + "\nconnection.toString()=" + connection.toString(), true);
            // 直接用XMPPTCPConnection发送
            connection.sendPacket(message);
        }
        catch (NotConnectedException e)
        {
            LogUtils.e(TAG, "sendTextMessage...发送消息抛出异常:" + "\ndwMessage.toString()=" + dwMessage.toString() + "\nconnection.toString()=" + connection.toString(), true);
            // 没有连接，处理
    ...
    private void getUserHistoryMsg(String dwID, String toID, long firstIndex, int chatType, final GetHistoryMsgCallback getHistoryMsgCallback)
    {
        ...
        LogUtils.d(TAG, "getUserHistoryMsg...begin:" + JSON.toJSONString(map));
        ...
        sendUrl.httpRequestWithParams(map, Constants.CONTEXTPATH + ConstantNet.API_SINGLE_HISTORY_MSG, Method.POST, new HttpCallBack()
        {
            ...
            public void onSuccess(String response, ResultBean msg)
            {
                LogUtils.d(TAG, "getUserHistoryMsg...response=" + response);
                if (msg.getResultCode() == 2222)
                {
            ...
            public void onFailure(String e)
            {
                LogUtils.d(TAG, "getUserHistoryMsg...onFailure,cause by:" + e);
    ...
    private void sendCardToE(String forwardDwId, String forwardName, String otherID, String otherNickname, float otherWorth, int userType, SendCardToListener listener)
    {
        ...
        long maxID = getTempMid(tempList);
        try
        {
