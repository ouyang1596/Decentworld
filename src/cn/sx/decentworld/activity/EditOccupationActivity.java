...
public class EditOccupationActivity extends BaseFragmentActivity implements
	...
	void init() {
		...
		oldOccupation = getIntent().getStringExtra("oldOccupation");
		LogUtils.i(TAG, "position=" + position + ",oldOccupation="
				+ oldOccupation);
		if (!oldOccupation.equals("")) {
			etOccupation.setText(oldOccupation);
		}
	...
	public void onClick(View v) {
		switch (v.getId()) {
		case R.id.tv_finish: {
			String newOccupation = etOccupation.getText().toString();
			if (!newOccupation.equals(oldOccupation)) {
				Intent intent = new Intent();
				intent.putExtra("newOccupation", newOccupation);
				intent.putExtra("position", position);
				setResult(RESULT_OK, intent);
				LogUtils.i(TAG, "修改行业为：" + newOccupation);
			} else {
