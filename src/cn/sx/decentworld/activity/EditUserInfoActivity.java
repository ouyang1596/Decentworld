...
public class EditUserInfoActivity extends BaseFragmentActivity implements OnClickListener
{
	...
	void init()
	{
		...
		if (userInfo != null)
		{
			LogUtils.i(TAG, "额外三张照片路径为：userInfo.getIcon=" + userInfo.getIcon() + ",userInfo.getIcon2=" + userInfo.getIcon2() + ",userInfo.getIcon3=" + userInfo.getIcon3());
			if (CommUtil.isNotBlank(userInfo.getIcon()))
			{
				ImageLoaderHelper.mImageLoader.displayImage(userInfo.getIcon(), iv_edit_user_info_icon_1, ImageLoaderHelper.mOptions);
			}
			if (CommUtil.isNotBlank(userInfo.getIcon2()))
			{
				ImageLoaderHelper.mImageLoader.displayImage(userInfo.getIcon2(), iv_edit_user_info_icon_2, ImageLoaderHelper.mOptions);
			}
			if (CommUtil.isNotBlank(userInfo.getIcon3()))
			{
				ImageLoaderHelper.mImageLoader.displayImage(userInfo.getIcon3(), iv_edit_user_info_icon_3, ImageLoaderHelper.mOptions);
			}
		}
		else
		{
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data)
	{
		...
		if ((resultCode == RESULT_OK) && (data != null))
		{
			if ((requestCode == REQUESTCODE_GET_ICON_1) || (requestCode == REQUESTCODE_GET_ICON_2) || (requestCode == REQUESTCODE_GET_ICON_3))
			{
				...
				{
					ImageLoaderHelper.clearCacheByUrl(UserInfoManager.getUserInfoInstance().getIcon());
				...
				{
					ImageLoaderHelper.clearCacheByUrl(UserInfoManager.getUserInfoInstance().getIcon2());
				...
				{
					ImageLoaderHelper.clearCacheByUrl(UserInfoManager.getUserInfoInstance().getIcon3());
