...
@EActivity(R.layout.activity_edit_mobile)
public class EditPhoneActivity extends BaseFragmentActivity implements OnClickListener, TextWatcher, OnEnsureClickListener {
	...
	public void init() {
		...
		relEditPhone.setBackgroundResource(R.color.new_blue_default);
	...
	private void showGetIdentifyingCodeDialog() {
		...
		gif.setOnEnsureClickListener(EditPhoneActivity.this);
	...
	private void sendIdentifyingCode() {
		...
		checkPhoneCode(iCode);
		// registerComponent.identifyCode(map, mCheckCodeHandle);
	}

	private Handler mCheckPhoneHandler = new Handler() {
		public void handleMessage(Message msg) {
			try {
				...
				intent.putExtra("newData", etMobile.getText().toString());
	...
	private void checkPhoneCode(String iCode) {
		...
		mSendUrl.httpRequestWithParams(map, Constants.CONTEXTPATH + ConstantNet.API_EDIT_PHONENUM, Method.POST,
				new HttpCallBack() {

					@Override
					public void onSuccess(String resultJSON, ResultBean resultBean) {
						LogUtils.d(TAG, "checkPhoneCode " + resultJSON);
						if (2222 == resultBean.getResultCode()) {
							Message message = mCheckPhoneHandler.obtainMessage();
							message.obj = resultBean.getData();
							mCheckPhoneHandler.sendMessage(message);
						} else {
