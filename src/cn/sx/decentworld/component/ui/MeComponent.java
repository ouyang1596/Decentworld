...
public class MeComponent implements OnClickListener {
	...
	public void onClick(View v) {
		...
		// 跳转到用户详细资料界面
		{
			...
			fragmentTransaction.replace(R.id.ll_chat_me_container,
					ChatFragment.userDetailFragment);
	 ...
	 * 初始化个人详细信息，从数据库拿取
	 */
	public void initUserInfoDatas(List<EditUserInfoItem> userInfoDatas) {
		userInfoDatas.clear();
		...
		if (userInfo != null && displayAuthority != null) {
			infoItem = new EditUserInfoItem(displayAuthority.getShowSign(),
					"sign", "个性签名", "", userInfo.getSign());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(displayAuthority.getShowRealName(),
					"realName", "基本信息", "实名", userInfo.getRealName());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(displayAuthority.getShowNickName(),
					"nickname", "基本信息", "昵称", userInfo.getNickName());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(displayAuthority.getShowAge(),
					"age", "基本信息", "年龄", userInfo.getAge() + "");
			userInfoDatas.add(infoItem);
			if (userInfo.getSex() == 0) {
				infoItem = new EditUserInfoItem(displayAuthority.getShowSex(),
						"sex", "基本信息", "性别", "女");
				userInfoDatas.add(infoItem);
			...
			infoItem = new EditUserInfoItem(displayAuthority.getShowPosition(),
					"position", "基本信息", "位置", userInfo.getPosition());
			userInfoDatas.add(infoItem);
			if (userInfo.getType() == 0) {
				infoItem = new EditUserInfoItem(displayAuthority.getShowType(),
						"type", "基本信息", "进入类型", "才");
				userInfoDatas.add(infoItem);
			...
			infoItem = new EditUserInfoItem(displayAuthority.getShowWealth(),
					"wealth", "基本信息", "身家", userInfo.getWealth());
			userInfoDatas.add(infoItem);

			infoItem = new EditUserInfoItem(false, "dwID", "基本信息", "DW号",
					userInfo.getDwID());
			userInfoDatas.add(infoItem);

			if (userInfo.getIfStudent().equals("0")) {
				infoItem = new EditUserInfoItem(
						displayAuthority.getShowOccupation(), "occupation",
						"基本信息", "行业", userInfo.getOccupation());
				userInfoDatas.add(infoItem);
				infoItem = new EditUserInfoItem(
						displayAuthority.getShowCorporation(), "corporation",
						"基本信息", "公司", userInfo.getCorporation());
				userInfoDatas.add(infoItem);
				infoItem = new EditUserInfoItem(displayAuthority.getShowjob(),
						"job", "基本信息", "职位", userInfo.getJob());
				userInfoDatas.add(infoItem);
				infoItem = new EditUserInfoItem(
						displayAuthority.getShowSchool(), "school", "我的历史",
						"毕业学校", userInfo.getSchool());
				userInfoDatas.add(infoItem);
				infoItem = new EditUserInfoItem(
						displayAuthority.getShowDepartment(), "department",
						"我的历史", "毕业院系", userInfo.getDepartment());
				userInfoDatas.add(infoItem);
			} else if (userInfo.getIfStudent().equals("1")) {
				infoItem = new EditUserInfoItem(
						displayAuthority.getShowSchool(), "school", "基本信息",
						"学校", userInfo.getSchool());
				userInfoDatas.add(infoItem);
				infoItem = new EditUserInfoItem(
						displayAuthority.getShowDepartment(), "department",
						"基本信息", "院系", userInfo.getDepartment());
				userInfoDatas.add(infoItem);
				infoItem = new EditUserInfoItem(
						displayAuthority.getShowClasses(), "classes", "基本信息",
						"班级", userInfo.getClasses());
				userInfoDatas.add(infoItem);
			...
			infoItem = new EditUserInfoItem(displayAuthority.getShowHometown(),
					"hometown", "我的历史", "故乡", userInfo.getHometown());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(displayAuthority.getShowNation(),
					"nation", "我的历史", "民族", userInfo.getNation());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(displayAuthority.getShowPhoneNum(),
					"phoneNum", "我的历史", "电话", userInfo.getPhoneNum());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(displayAuthority.getShowEmail(),
					"email", "我的历史", "邮箱", userInfo.getEmail());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(displayAuthority.getShowQQ(), "qq",
					"我的历史", "QQ", userInfo.getQq());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(displayAuthority.getShowWechat(),
					"wechat", "我的历史", "微信", userInfo.getWechat());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(displayAuthority.getShowBlog(),
					"blog", "我的历史", "微博", userInfo.getBlog());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(displayAuthority.getShowVehicle(),
					"vehicle", "我的历史", "车子", userInfo.getVehicle());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(
					displayAuthority.getShowBloodType(), "bloodType", "我的历史",
					"血型", userInfo.getBloodType());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(
					displayAuthority.getShowConstellatory(), "constellatory",
					"我的历史", "星座", userInfo.getConstellatory());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(
					displayAuthority.getShowMaritalStatus(), "maritalStatus",
					"我的历史", "婚否", userInfo.getMaritalStatus());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(displayAuthority.getShowReligion(),
					"religion", "我的历史", "宗教", userInfo.getReligion());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(
					displayAuthority.getShowSpeciality(), "speciality", "我的历史",
					"特长", userInfo.getSpeciality());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(displayAuthority.getShowIdol(),
					"idol", "我的历史", "偶像", userInfo.getIdol());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(displayAuthority.getShowMotto(),
					"motto", "我的历史", "座右铭", userInfo.getMotto());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(
					displayAuthority.getShowFootprint(), "footprint", "我的历史",
					"足迹", userInfo.getFootprint());
			userInfoDatas.add(infoItem);

			infoItem = new EditUserInfoItem(
					displayAuthority.getShowLikebooks(), "likebooks", "我的爱好",
					"喜欢的书", userInfo.getLikebooks());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(
					displayAuthority.getShowLikemusic(), "likemusic", "我的爱好",
					"喜欢的音乐", userInfo.getLikemusic());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(
					displayAuthority.getShowLikesmovies(), "likesmovies",
					"我的爱好", "喜欢的电影", userInfo.getLikesmovies());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(
					displayAuthority.getShowLikesport(), "likesport", "我的爱好",
					"喜欢的运动", userInfo.getLikesport());
			userInfoDatas.add(infoItem);
			infoItem = new EditUserInfoItem(displayAuthority.getShowCate(),
					"cate", "我的爱好", "美食", userInfo.getCate());
			userInfoDatas.add(infoItem);
		}
	}
	...
	public void getIconPath(String dwID) {
		...
		String iconSmall = ImageUtils
				.getIconByDwID(dwID, ImageUtils.ICON_SMALL);
		String iconMain = ImageUtils.getIconByDwID(dwID, ImageUtils.ICON_MAIN);
		String iconExtra1 = ImageUtils.getIconByDwID(dwID,
				ImageUtils.ICON_EXTRA_1);
		String iconExtra2 = ImageUtils.getIconByDwID(dwID,
				ImageUtils.ICON_EXTRA_2);
		LogUtils.i(TAG, "iconSmall = " + iconSmall + ",iconMain=" + iconMain
				+ ",iconExtra1=" + iconExtra1 + ",iconExtra2=" + iconExtra2);
		// 重新获取四张图片的地址并保存在数据库中
		// toast.show("id="+dwID);
